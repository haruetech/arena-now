...
/**
 * PC one-view dashboard.
 *
 * The approved reference is 3:2.  Keeping the same aspect ratio is important:
 * forcing it into a 16:9 / viewport-height crop hides TODAY SHOW, LIVE NOW and
 * AI recommendation cards.  This component therefore scales the whole board
 * to fit the browser height and keeps every hotspot aligned with the image.
 */
const hotspots = [
  // header
  ...
  { label: "입장게이트", href: "#live-issues", left: 67.5, top: 25.2, width: 11.0, height: 10.5 },
...
export default function DesktopDashboard() {
  return (
    <section
      ...
      className="relative hidden w-full overflow-hidden bg-[#020916] lg:block"
      aria-label="ARENA NOW 메인 대시보드"
    >
      <div className="flex min-h-[calc(100vh-8px)] w-full items-start justify-center bg-[#020916] px-2 py-2 2xl:px-4">
        <div className="arena-dashboard-board relative aspect-[3/2] w-auto max-w-full overflow-hidden bg-[#020916]">
          <img
          ...
          {/* Reference image weather is replaced by live KMA data. */}
          <div className="absolute right-[2.2%] top-[1.1%] z-30">
            <WeatherWidget desktop />
          </div>

          <nav aria-label="메인 대시보드 바로가기" className="absolute inset-0 z-20">
            {hotspots.map((item) => (
              <a
                ...
                title={item.label}
                className="absolute rounded-2xl outline-none transition hover:bg-cyan-300/[.07] focus-visible:bg-cyan-300/10 focus-visible:ring-2 focus-visible:ring-cyan-300/80"
      ...
      <div className="mx-auto flex max-w-[1540px] items-center justify-between gap-4 border-y border-cyan-300/10 bg-[#06111e] px-6 py-3 text-[11px] text-white/55">
        ...
        <a href="#service-detail" className="relative z-20 font-bold text-cyan-300 hover:text-white">
          상세 서비스 보기 ↓
        </a>
      </div>
    </section>
