...
## 2026-09-08 Premium Hero / Editable Copy
- 기존 추상 ArenaSkyline 중심 Hero를 조감도 중심 Premium Hero로 교체했습니다.
- 우측 하단 톱니바퀴(⚙)에서 히어로 문구, 메뉴명, 핫스팟 라벨, 버튼, 하단 슬로건, 배경 이미지 URL을 각각 수정할 수 있습니다.
- 저장값은 현재 프로토타입에서 브라우저 localStorage에 저장됩니다. 실제 운영 시 Supabase/Admin DB로 교체하는 것을 권장합니다.
- 포함된 조감도 이미지는 생성형 AI로 만든 컨셉 이미지이며 실제 서울아레나 공식 조감도를 복제한 파일이 아닙니다. 화면 하단의 컨셉 고지 문구를 유지하세요.
